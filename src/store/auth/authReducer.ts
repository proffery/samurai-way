...
import { initializeApp, addAppAlert, SetAppIsLoading, SetAppIsInitialized, setAppIsLoading } from "store/app/appReducer"
...
//ACTIONS
export const setAuthUserData = (data: GetMeData) => ({ type: SET_AUTH_DATA, payload: { data } }) as const
export const setIsLoggedIn = (value: boolean) => ({ type: SET_IS_LOGGED_IN, payload: { value } }) as const
export const setAuthUserPhoto = (photoUrl: string) => ({ type: SET_PHOTO_URL, payload: { photoUrl } }) as const
export const setCaptchaUrl = (captchaUrl: string) => ({ type: SET_CAPTCHA_URL, payload: { captchaUrl } }) as const
export const cleanReducer = () => ({ type: CLEAR_REDUCER }) as const
