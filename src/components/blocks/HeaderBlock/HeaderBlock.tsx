    ...
    <S.HeaderBlock id="profile-header" className={className}>
      ...
      <S.InfoConainer>
        <S.TextContainer>
          ...
          {authId === userId ? (
            <HeaderBlockForm addAppAlert={addAppAlert} changeProfileStatus={changeProfileStatus} status={status} />
          ) : (
            <span>{status}</span>
          )}
        </S.TextContainer>
          ...
          <S.ButtonsContainer>
            <S.UploadForm>
              <S.UploadButton htmlFor={"photo-upload"} title="Uplod photo">
                <input id={"photo-upload"} type={"file"} accept={"image/*"} onChange={uploadPhotoHandler} />
