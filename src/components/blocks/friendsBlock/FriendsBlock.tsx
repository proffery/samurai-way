...
export const FriendsBlock: React.FC<Props> = memo(({ headerName: blockHeaderName, className, isFriends = true }) => {
    ...
    const { totalUsersCount, currentPage, usersOnPage, users } = useSelector(
        isFriends ? selectFriendsData : selectPossibleFriendsData,
    )
    ...
    return (
        <S.FriendsBlock id={blockHeaderName.toLowerCase().replaceAll(" ", "-")} className={className}>
            <FlexWrapper justify={"space-between"}>
                <Button onClick={randomizeFriends} variant={"link"} ariaLabel={"Randomize button"}>
                    <BlockHeader>{blockHeaderName}</BlockHeader>
                </Button>
            </FlexWrapper>
            <S.FriendsList>
                {users.map((friend) => (
